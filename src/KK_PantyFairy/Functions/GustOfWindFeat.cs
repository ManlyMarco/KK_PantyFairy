    ...
    public static class GustOfWindFeat
    {
        ...
        public static bool Enabled
        {
            ...
            set
            {
                ...
                if (value)
                {
                    ...
                    var ico = GameAPI.AddTouchIcon(PantyFairyPlugin.GetSprite("touch_icon_wind.png"), button => button.onClick.AddListener(Start), 1, 690);
        ...
        private static IEnumerator StartCo(TalkScene talkScene)
        {
            ...
            if (!Enabled || heroine == null)
            {
                ...
                yield return EventUtils.StartTextSceneEvent(talkScene, list);
                yield break;
            }
            ...
            if (PantyFairyPlugin.IsHentStatBelow(70))
            {
                var list = EventUtils.CreateNewEvent();
                list.Add(Program.Transfer.Text(EventUtils.Narrator, "Argh, I can feel the power but it's hard to focus it. Looks like I need to improve my imagination some more."));
                list.Add(Program.Transfer.Text(EventUtils.Narrator, "(You need at least 70 Sensuality/H points to use this skill)"));
                list.Add(Program.Transfer.Close());
                yield return EventUtils.StartTextSceneEvent(talkScene, list);
                yield break;
            }
